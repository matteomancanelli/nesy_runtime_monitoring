...
def load_timing(csv_paths: str | Path | list) -> pd.DataFrame:
    ...
    # Phase E0 schema migration. Historical July CSVs used ``n_leaves`` for
    # atoms in exp2 and overwrote it with |Q| in exp6/7. Preserve those files as
    # archaeology while presenting named axes to all plotting code.
    legacy_state_family = df["formula_name"].str.startswith(
        ("boundedresp_", "kthlast_")
    )
    if "n_leaves" in df:
        legacy_atoms = pd.Series(
            np.where(legacy_state_family, 2, df["n_leaves"]), index=df.index
        )
        legacy_dfa_states = pd.Series(
            np.where(legacy_state_family, df["n_leaves"], np.nan), index=df.index
        )
        if "n_atoms" not in df:
            df["n_atoms"] = legacy_atoms
        else:
            df["n_atoms"] = df["n_atoms"].fillna(legacy_atoms)
        if "dfa_states" not in df:
            df["dfa_states"] = legacy_dfa_states
        else:
            df["dfa_states"] = df["dfa_states"].fillna(legacy_dfa_states)
...
def _memory_curves() -> tuple[list[int], list[float], list[float]]:
    ...
    for formula in IJCNN_SUITE:
        ...
        dense_bytes = n_q * n_q * (2**formula.n_atoms) * FLOAT_BYTES
        factored_bytes = (dt._cube_rt.numel() + dt._cube_rf.numel()) * FLOAT_BYTES
        mem_n.append(formula.n_atoms)
...
def plot_exp2(csv_paths=None, out_dir: Path | None = None) -> list[Path]:
    ...
    out_dir = out_dir or RESULTS_DIR
    all_n = sorted(df["n_atoms"].unique())
    outs = []

    # (1) time per cell
    fig, ax = _new_ax()
    _draw_timing(ax, df, "n_atoms")
    ...
    fig, ax = _new_ax()
    _draw_timing(ax, df, "n_atoms", ynorm="n_atoms")
    ax.set_xlabel("Leaves (atoms)")
    ...
    try:
        ...
        ax.axvline(DENSE_MAX_ATOMS, color=dense_c, ls="--", alpha=0.4)
        ax.set_yscale("log")
        ...
        ax.text(
            0.5,
            0.5,
            "memory panel needs ltlf2dfa + torch\n(recomputed "
            "analytically, not stored in the CSV)",
            ha="center",
            va="center",
...
def plot_exp3(csv_paths=None, out_dir: Path | None = None) -> list[Path]:
    ...
    for monitor in _ordered_monitors(df):
        ...
        for cfg in sorted(df.loc[df["monitor_name"] == monitor, "config"].unique()):
            ...
            ax.errorbar(
                g["n_traces"],
                g["mean_s_per_trace"] * 1e3,
                yerr=g["std_s_per_trace"] * 1e3,
                marker=mark[cfg],
                ms=6,
                lw=2,
                capsize=3,
...
def plot_device_comparison(
    csv_paths, experiment: str, out_dir: Path | None = None, monitors=None
) -> list[Path]:
    ...
    if len(configs) < 2:
        print(
            f"[{experiment}] only one config present ({configs}); device "
...
def plot_device_speedup(
    csv_paths, experiment: str, out_dir: Path | None = None
) -> list[Path]:
    ...
    if "CPU" not in set(df["config"]) or not gpu_cfgs:
        print(
            f"[{experiment}] need both a CPU CSV and a GPU CSV for the "
    ...
    for monitor in _ordered_monitors(df):
        label, color = style_for(monitor, seen)
        cpu = df[(df["monitor_name"] == monitor) & (df["config"] == "CPU")].set_index(
            xcol
        )["mean_s_per_cell"]
        gpu = df[(df["monitor_name"] == monitor) & (df["config"] == gpu_cfg)].set_index(
            xcol
        )["mean_s_per_cell"]
...
def correctness_cost_table(csv_paths, xcol: str = "n_atoms") -> pd.DataFrame:
    ...
    for orig, corr, enc in _CORRECTNESS_PAIRS:
        for cfg in sorted(df["config"].unique()):
            o = df[(df["monitor_name"] == orig) & (df["config"] == cfg)].set_index(
                xcol
            )["mean_s_per_cell"]
            c = df[(df["monitor_name"] == corr) & (df["config"] == cfg)].set_index(
                xcol
            )["mean_s_per_cell"]
    ...
    if not out.empty:
        ...
        summary = (
            out.groupby(["encoding", "config"])["ratio"]
            .agg(["min", "median", "max"])
            .round(2)
        )
...
def plot_correctness_cost(
    ...
    for orig, corr, _enc in _CORRECTNESS_PAIRS:
        for monitor in (orig, corr):
            ...
            for cfg in sorted(df.loc[df["monitor_name"] == monitor, "config"].unique()):
                g = df[
                    (df["monitor_name"] == monitor) & (df["config"] == cfg)
                ].sort_values(xcol)
                lbl = f"{label} — {cfg}" if multi else label
                ax.errorbar(
                    g[xcol],
...
def plot_exp7_stateblowup(csv_paths=None, out_dir: Path | None = None) -> list[Path]:
    ...
    max_compiled_q = max(int(q) for q in df["dfa_states"].unique())
