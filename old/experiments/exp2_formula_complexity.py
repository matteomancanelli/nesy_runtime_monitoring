...
#     DENSE_MAX_ATOMS and visualize the memory wall analytically (3rd panel),
...
# DeepDFA-dense's alphabet wall and is capped at DENSE_MAX_ATOMS for the same
...
with tqdm(total=total, desc="exp2") as pbar:
    for monitor_cls in MONITORS:
        for formula in IJCNN_SUITE:
            ...
            alphabet_oom = (
                monitor_cls in _ALPHABET_WALL and formula.n_atoms > DENSE_MAX_ATOMS
            )
            if alphabet_oom:
                pbar.set_postfix(
                    monitor=monitor_cls.__name__, n=formula.n_atoms, skip="OOM"
                )
                pbar.update()
                continue
            key = result_key(monitor_cls.__name__, formula.name, TRACE_LENGTH, N_TRACES)
            if key in completed:
                pbar.set_postfix(
                    monitor=monitor_cls.__name__, n=formula.n_atoms, skip=True
                )
                pbar.update()
                continue
            # label BEFORE the call: one time_monitor can run for minutes and the
            # bar only advances after it returns.
            pbar.set_postfix(monitor=monitor_cls.__name__, n=formula.n_atoms, run="...")
            r = time_monitor(
                monitor_cls,
                formula,
            ...
            pbar.set_postfix(monitor=monitor_cls.__name__, n=formula.n_atoms)
            pbar.update()
...
print()
_cols = ["monitor_name", "n_atoms", "mean_s_per_cell", "std_s_per_cell"]
