...
def nested_x(base: BenchmarkFormula, depth: int) -> BenchmarkFormula:
    ...
    return BenchmarkFormula(
        ...
        atoms=base.atoms,
        family="nested_x",
        source="project temporal-depth ablation",
        parameters=(("temporal_depth", depth),),
        tree_shape=base.tree_shape,
        roles=("legacy_depth_ablation",),
    )
...
with tqdm(total=total, desc="exp5") as pbar:
    for monitor_cls in MONITORS:
        for depth in DEPTHS:
            ...
            if key in completed:
                pbar.set_postfix(monitor=monitor_cls.__name__, depth=depth, skip=True)
                pbar.update()
                continue
            # label BEFORE the call: one time_monitor can run for minutes and the
            # bar only advances after it returns.
            pbar.set_postfix(monitor=monitor_cls.__name__, depth=depth, run="...")
            r = time_monitor(
                monitor_cls,
                formula,
