...
> ⚠ **Measured 2026-07-10 (exp6, `bounded_response`) — the residual state space is wildly non-canonical, which is the sharper form of this wall.** `simplify` (sympy `simplify_logic` → `POSform`) does **not** collapse progression-equivalent residuals, so the residual DFA explodes while the minimal DFA grows only *linearly*: at deadline k = 8 / 10 / 12 / 14 the residual `n_states` is 38 / 711 / 3776 / 16064 against a minimal `|Q|` of 10 / 12 / 14 / 16. Build time follows (2 s / 10 s / 33 s / 191 s, ≈5× per +2 in k), entirely inside `compile()` before a single cell is timed, dominated by one `simplify_logic` per residual (58 s of 93 s at k=12). The honest claim is therefore "residual closure ≫ |Q| on deep nesting", not "≈ |Q|". Mitigations in place: `build_progression_dfa` is `lru_cache`d (frozen dataclass, safe to share) so the flat and structured monitors pay the BFS once rather than twice, and exp6 caps both progression monitors at `PROGRESSION_MAX_Q = 14`.
>
> **Open question before this is reported as a paradigm property:** is the blowup intrinsic to progression, or an artifact of using sympy's DNF/POS simplifier as the residual canonicalizer? A canonical key (BDD, or `simplify` to a normal form with a real equivalence check) would plausibly collapse 16064 back toward 16. Worth one experiment — it changes whether paradigm 2's "cost of correctness" is a fundamental wall or an implementation detail.
