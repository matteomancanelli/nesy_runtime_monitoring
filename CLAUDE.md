...
- **0.4 — Re-run Exp 3 on a real GPU (Google Colab).** The 4 GB / 70 W laptop GPU saturates at tiny batch and undersells every GPU path. **Hardware: the user runs the sweeps on Google Colab** (CPU runtime and a GPU runtime, usually a Tesla T4); local dev is the `nesy-monitoring` conda/pip env. **There is no Docker in this project — do not reference a Docker server.** Re-run on Colab (and optionally with a larger automaton to give batched DeepDFA room). Honest outcome either way: a modest real win, or "GPU advantage needs larger automata/hardware." (Code is GPU-ready: experiments auto-select `device="cuda"`; each row is stamped with `device`/`gpu_name`, and the resume key is device-aware so CPU and GPU runs coexist for the CPU-vs-GPU figures.)
...
- **0.6 — Two directions to invert the "symbolic always wins" trend. ✅ (code done + verified; GPU re-run pending).** The first-pass finding is that pure symbolic beats every NeSy paradigm on crisp throughput — *expected* (a DFA walk is a dict lookup at the native-code floor; the NeSy paths pay tensor-framework/kernel-launch overhead per timestep on a non-parallelizable time recurrence with trivial per-step work). Decomposing Exp 3 batch=1 vs 1024 shows **~83 µs of fixed per-call overhead per cell vs ~1.5 µs of actual compute** — the overhead, not the arithmetic, is what loses. Two orthogonal levers, each for a different regime, are now implemented so we can test whether *anything* survives:
  ...
  - **Note (labeling wart):** `SymbolicDFAMonitor` ignores `device` — it is always a CPU dict walk. So the symbolic rows in a GPU CSV are stamped `device=cuda`/`gpu_name=Tesla T4` but actually ran on the CPU; that is why symbolic has "no overhead" (it never enters torch or launches a kernel), not because it is fast *on* the GPU.
