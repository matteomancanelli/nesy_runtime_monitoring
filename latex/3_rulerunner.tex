...
\begin{proof}
Let $\boldsymbol{\sigma}=(\sigma_0,\ldots,\sigma_{T-1})$ be a finite  trace. For $0\leq t<T$, let $\Sigma_t$ be the active root set before reading cell $\sigma_t$, that is, after the prefix $\sigma_0\cdots\sigma_{t-1}$ has been consumed. Let
\[
...
We first use the defining property of progression. For every formula $\chi$, every non-final cell $s$, and every  finite continuation $\boldsymbol{\tau}$,
\begin{equation}
...
\end{equation}
for every $0\leq t<T$ and every  finite continuation $\boldsymbol{\tau}$.
