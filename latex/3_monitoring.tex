...
\paragraph{Architecture.}
DeepDFA \cite{deepdfa_ecai2024} keeps the automaton of the symbolic paradigm but replaces its explicit state walk with a differentiable, tensorized one; we adopt the same forward formulation used for predictive monitoring of business processes by \cite{mezini2025neuro}. Starting again from the minimal DFA $A_\varphi = (2^\Sigma, Q, q_0, \delta, F)$ of Section~\ref{sec:ltlf}, we encode its transition function as a three-way tensor $T \in \{0,1\}^{|Q| \times |2^\Sigma| \times |Q|}$ with $T[q, \sigma, q'] = 1$ iff $\delta(q, \sigma) = q'$, and the accepting set as an indicator vector $v_o \in \{0,1\}^{|Q|}$ with $v_o[q] = 1$ iff $q \in F$. The monitor state is no longer a single automaton state but a row vector $\mathbf{q}_t \in [0,1]^{|Q|}$ over states, initialized to the one-hot vector $\mathbf{q}_0 = v_i$ of $q_0$. Writing $T_\sigma = T[\,\cdot\,, \sigma, \cdot\,] \in \{0,1\}^{|Q| \times |Q|}$ for the (row-stochastic) transition matrix of symbol $\sigma$, a step is a single vector--matrix product
\begin{equation}
