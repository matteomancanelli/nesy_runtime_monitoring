...
%
As in the original system, evaluating a formula at a time point is a bottom-up computation over a parsing tree, with the only difference that here we consider the subformulas of a residual instead of $\varphi$. In details, the RuleRunner state $S_t$ at time $t$ holds the observations of $\sigma_t$, the active rules $R[\psi]$, and the truth evaluations $[\psi]V$ for $\psi\in sub(\rho_t)$. 
What is carried across time is the residual alone. Note that since only the residual is progressed, the undecided subformulas $[\psi]?$ of $\rho_t$ are not reactivated as before, but flushed as every other truth evaluation.
...
T\]
where $\mathsf{split}_\wedge(\top)=\emptyset$, $\mathsf{split}_\wedge(\chi_1\land\chi_2) = \mathsf{split}_\wedge(\chi_1)\cup\mathsf{split}_\wedge(\chi_2)$, and any formula whose top-level operator is not a conjunction is kept as a single element. Progression is a homomorphism for conjunction, so the elements of $\Sigma_t$ may be progressed independently and re-collected, leaving the update above unchanged. 
...
As described above, we normalize residual formulas after progression.  This is not required for semantic soundness, but raw syntax may then grow without bound. For instance, repeated failure of $\psi$ in $F\psi$ would produce $\bot\lor F\psi$, then $\bot\lor(\bot\lor F\psi)$, and so on. Normalization removes this irrelevant growth and makes the formula easier to read and interpret, %Typically, the normalization includes pushing negations through temporal and boolean operators, removes repetitions, simplifies subformulas including $\top$ and $\bot$, and so on.
while exposing more top-level conjuncts to $\mathsf{split}_\wedge$. Note that this normal form is deliberately syntactic, since it does not perform a semantic equivalence check. Distinct residuals may therefore be collapsed furtherly, and the closure size is in general larger than the minimal-DFA size.
